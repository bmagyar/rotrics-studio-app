...
class Index extends React.Component {
    ...
    actions = {
        ...
        //xyz
        xPlus: () => {
            this.actions._move(`G1 X${this.state.step}`)
        },
        xMinus: () => {
            this.actions._move(`G1 X${-this.state.step}`)
        },
        yPlus: () => {
            this.actions._move(`G1 Y${this.state.step}`)
        },
        yMinus: () => {
            this.actions._move(`G1 Y${-this.state.step}`)
        },
        zPlus: () => {
            this.actions._move(`G1 Z${this.state.step}`)
        },
        zMinus: () => {
            this.actions._move(`G1 Z${-this.state.step}`)
        },
        z0: () => {
            this.props.startTask("G1 Z0", false)
        },
        //others
        home: () => {
        ...
        leftTop: () => {
            this.actions._move(`G1 X${-this.state.step} Y${this.state.step}`)
        },
        leftBottom: () => {
            this.actions._move(`G1 X${-this.state.step} Y${-this.state.step}`)
        },
